...
document.getElementById('webCreationBtn').addEventListener('click', ()=> {
    navigateTo('website-creation.html');
});
    ...
    document.getElementById('content').innerHTML += `
        <div>
            <div id='serviceDiv${id}' class='hidden'>
                ${img ? `<img id='service${id}' loading='lazy' src="${img}" alt="${title}" width="150"/>` : ''}
            </div>
            <h3>${title}</h3>
            ${desc ? `<p>${desc}</p>` : ''}
        </div>
